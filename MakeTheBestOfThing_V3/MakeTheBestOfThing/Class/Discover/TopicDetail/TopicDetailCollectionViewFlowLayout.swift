...
class TopicDetailCollectionViewFlowLayout: UICollectionViewFlowLayout {
    ...
    override func prepareLayout() {
        ...
        let itemWidth = (kScreenWidth - (CGFloat(rowItemNum - 1) * minimumInteritemSpacing)
               - sectionInset.left - sectionInset.right) / CGFloat(rowItemNum)
        
        /** 计算所有item的布局属性 */
        calculateAllItemLayoutAttributeByItemwidth(itemWidth)
...
/** 计算 所有item的布局属性 */
    private func calculateAllItemLayoutAttributeByItemwidth(itemWidth : CGFloat) {
        ...
        let attribute = calculateItemLayoutAttribute(index++, itemWidth: itemWidth)
    ...
    /** 计算一个item 的布局属性 */
    private func calculateItemLayoutAttribute(index:Int,itemWidth:CGFloat) -> UICollectionViewLayoutAttributes
    {
     ...
     // 宽度最小的序列号和列宽
        let colAndHeght = findMinHeightColIndexAndHeight()
        
        let x = sectionInset.left + CGFloat(colAndHeght.col) * (itemWidth + minimumInteritemSpacing)
        let y = colAndHeght.height
        let h = calculateItemHeight(index, width: itemWidth)
        
        // 将item的高度添加到数组进行记录
        colItemHeightValues![colAndHeght.col] += (h + minimumLineSpacing)
    ...
    /**
     ...
     :returns: item的高度
     */
    private func calculateItemHeight(index: Int,width: CGFloat ) -> CGFloat {
